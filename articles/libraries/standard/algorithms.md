...
Az általános rutin (`AmpAmpObliviousByReflectionPhases`) két regisztert tartalmaz, amelyeket `ancillaRegister` és `systemRegister`hívunk. Emellett két Oracle-t is elfogad a szükséges tükrözésekhez. A `ReflectionOracle` csak a `ancillaRegister` működik, míg a `ObliviousOracle` mindkét regiszteren közösen működik. A (`ancillaRegister`) bemenetét inicializálni kell az első reflexiós operátor ($ \boldone-2P_1 $) 1 eigenstate.
...
A hozzávetőleges QFT működéséhez a dyadic $Z $-forgási művelet <xref:microsoft.quantum.primitive.rfrac> és a <xref:microsoft.quantum.intrinsic.h> művelet szükséges.
...
Ismert, hogy a $k \ge \log_2 (n) + \log_2 (1/\epsilon) + $3. egy kötve $\\| \operatorname{QFT} – \operatorname{AQFT} \\| < \epsilon $.
...
Egy olyan kiegészítőkre alakítható át, amely két kvantum-bemenettel rendelkezik a $ $ \operatorname{Add}\ket{a}\ket{b} = \ket{a}\ket{a + b}, $ $ $n használatával, amely a \begin{align} \operatorname{Add} \ket{a} \ket{b} & = \Lambda\_{a\_0} \left (\operatorname{Add} (1) \right) \Lambda\_{a\_1} \left (\operatorname{Add} (2) \right) \Lambda\_{a\_2} \left (\operatorname{Add} (4) \right) \cdots \Lambda\_{a\_{n-1}} \left (\ operatorname {Add} ({{n-1}}) \right) \ket{a}\ket{b} \\\\ & = \ket{a} \ket{b + a}, \end{align} $n $ bites egész számra $a $ és $b $, valamint a többtényezős $2 ^ n $ értéket.  Ne felejtse el, hogy a $ \Lambda\_x (A) $ jelölés a ($A $) művelethez a művelet ellenőrzött verziójára vonatkozik a qubit $x $ as vezérlővel.
...
$ $ Ha definiáljuk a $ $ \ket{\phi\_k (a)} = \frac{1}{\sqrt{2}} \left (\ket{0} + e ^ {i2\pi a/2 ^ k} \ket{1} \right), $ $, majd néhány algebra után láthatja, hogy $ $ \operatorname{QFT}\ket{a} = \ket{\phi\_1 (a)} \otimes \cdots \ otimes \ket{\phi\_n (a)}.
...
### <a name="quantum-phase-estimation"></a>Quantum fázis becslése ###
...
\end{align} gyűjthetjük a \begin{align} V (\ket{+} \otimes \ket{\phi}) & = \frac{\ket{0} + e ^ {i \phi} \ket{1}} {\sqrt{2}} \otimes \ket{\phi} \\\\ & = (R_1 (\phi) \ket{+}) \otimes \ket{\phi} \end{align}, ahol a $R _1 $ a <xref:microsoft.quantum.intrinsic.r1> művelet által alkalmazott egységes alkalmazás.
...
Így továbbra is regisztrálhat a \begin{align} \ket{\psi} & = \sum_{j = 0} ^ n R_1 (2 ^ j \phi) \ket{+} \\\\ & \propto \bigotimes_{j = 0} ^ {n} \left (\ket{0} + \exp (i 2 ^ {j} \phi) \ket{1}\right) @no__ t_4_ \\ & \propto \sum_{k = 0} ^ {2 ^ n-1} \exp (i \phi k) \ket{k} \end{align} ahol a $n $ érték a szükséges bitek száma, és hol használták a ${} \propto {}$ értéket, jelezve, hogy letiltottuk a $ 1/\sqrt{2 ^ n} $.

Ha feltételezzük, hogy a $ \phi = 2 \pi p/2 ^ k $ egy egész $p $-ra, akkor ezt a következőt ismerjük el: $ \ket{\psi} = \operatorname{QFT} \ket{p_0 p_1 \dots p_n} $, ahol $p _j $ a $j ^ {\textrm{th}} $ bit of $2 \pi \phi $.
