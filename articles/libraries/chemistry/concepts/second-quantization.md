...
Példa a második kvantálás működésére: tegyük fel, hogy a $ \psi_0\cdots \psi_{N-1} $ a térbeli pályák orthonormal készlete.
...
Ez azt jelenti, hogy az érvényes állapot a $ \psi_{0, a \uparrow}, a \ldots, a \psi_{N-1, a \uparrow}, a \psi_{0, a \downarrow}, a \ldots, a \psi_{N-1, a \downarrow} $, ahol a $ \uparrow $ és a $ \downarrow $ a spin foka két eigenstates adja meg Szabadságfok.
...
Ez azt jelenti, hogy formálisan gondoljuk át az Wave-függvény spin-és térbeli részét is, mint $ \psi_{0} \cdots \psi_{2N-1} $, ahol az egyes indexek mostantól egy $ (j, \sigma) $ enumerálást jelentenek.
...
Ezeket a $a ^ \dagger_j $ és $a _j $ jelöli.
...
Az ilyen operátorok is használhatók a $ $ \ket{0} \ket{1} \ket{1} \ket{0} = a ^ \dagger_1 a ^ \dagger_2 \ket{0}^ {\otimes 4}.
...
\end{Equation} ez az operátor Megszámolja egy adott spin-orbitális foglalkozását, ami azt jelenti, hogy a \begin{align} n_i \ket{0}_i & = 0 \ Number\\\
...
$ $ Ilyen operátorok azt mondják, hogy "anti-ingázás", és általánosságban minden $i, j $, hogy \begin{align} a ^ \dagger_i a ^ \dagger_j =-(1-\delta_{i, j}) a ^ \dagger_j a ^ \dagger_i, \quad a ^ \dagger_i a_j = \delta_{i, j}-a_j a ^ \dagger_i.
...
\begin{Equation} \hat{H} = \sum\_{pq} H\_{pq} a ^ \dagger\_p a\_q + \frac{1}{2}\sum\_{pqrs} H\_{pqrs} a ^ \dagger\_p a ^ \dagger\_q a\_ra\_s + h\_{\textrm NUC}, \label{EQ: totalHam} \end{Equation}, ahol $h\_{\textrm NUC} $ a nukleáris energia (amely állandó a Born-Oppenheimer közelítés alatt) és
...
\begin{align} h\_{pqrs} & = \int\_{-\infty} ^ \infty \int\_{-\infty} ^ \infty\psi\_p ^\*(x\_1) \psi\_q ^\*(x\_2) \left (\frac{1}{| x_1-x_2 |} \ jobbra) \psi\_r (x\_2) \psi\_s (x\_1) \mathrm{d} ^ 3x\_1 \ mathrm {d} ^ 3x\_2. \ Label {EQ: integrálja} \end{align}

Az $h\_{pq} $ kifejezéseket egy elektron-egységként tekintjük, mert minden ilyen kifejezés csak egyetlen elektront érint, és hasonlóképpen $h\_{pqrs} $ a két elektronos egység.
...
Hasonlóképpen, az $h _ {pqrs} kifejezés a ^ \dagger_p a ^ \dagger_q a_r a_s $ (a DISTINCT $p, a q, az r, a s $ esetében) két elektront mutat be a spin orbitális-ben $r $ és $s $ szétszóródás egymástól, és végül a spin orbiters $p $ és $q
...
$ $ \begin{align} H = \sum\_{pq} H\_{pq} a ^ \dagger\_{p} a\_{q} + \frac{1}{2}\sum\_{pqrs} H\_{pqrs} a ^ \dagger\_{p} a ^ \dagger\_{q} a @no__ t_10_ {r}\_{s}.
\end{align} $ $
...
Például abban az esetben, ha a $p, a q, az r, a s $ különálló indexeket használ, használhatjuk a következőt: $ $ a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{r} a\_{s} =-a ^ \dagger\_{q} a ^ \dagger\_{ p} a\_{r} a\_{s} =-a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{s} a\_{r} = a ^ \dagger\_{q} a ^ \dagger\_{p} a\_{s} a\_{r}.
$$

Továbbá, mivel $H $ Hermitian, minden nem Hermitian fermionic-kezelő, mondjuk $h\_{pqrs} ^ \dagger\_{p} a ^ \dagger\_{q} a\_{r} a\_{s} $ Hermitian konjugátumot tartalmaz, amely $H $-ben is megtalálható. Az ezekkel a symmetries jellemezhető kifejezések csoportjának egyedi indexeléséhez definiálunk egy kanonikus sorrendet az indexek esetében (i\_1, \cdots, i\_n, j\_1, \cdots, j\_m) $ $n + m $ fermionic operátorok $a ^ \dagger\_{i\_1} \cdots ^ \dagger\_{i\_n} a\_{j\_1} \cdots a\_{j\_m} $as követi:
