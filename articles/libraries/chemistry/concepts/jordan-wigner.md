...
Ne felejtse el, hogy a $ \ket{0}_j $ érték azt jelenti, hogy a spin orbitális $j $ üres, és a $ \ket{1}_j $ azt jelenti, hogy foglalt.
...
Könnyen ellenőrizhető, hogy a \begin{align} ^ \ dagger_j & = \begin{bmatrix}0 & 0 \\\ 1 & 0 \end{bmatrix} = \frac{X_j-iY_j}{2}, \nonumber\\\\ a_j & = \begin{bmatrix}0 & 1 \\\ 0 & 0 \end{bmatrix} = \frac{X_j + iY_j}{2}, \end{align} ahol $X _j $ és $Y _j $ a qubit $X $-on működő Pauli-$Y $ és-$j $ operátorok.

>[!NOTE]
> A Q # a $ \ket{0}$ állapot a $Z $ operátor + 1 eigenstate jelöli. A fizika egyes területein a $ \ket{0}$ az alacsony energiájú terepi állapotot jelöli, így az $Z $ operátor-1 eigenstate. Ezért előfordulhat, hogy egyes képletek eltérhetnek a népszerű irodalomtól.

A kémia könyvtárában a $ \ket{0}$ értéket használjuk a nem foglalt spin-Orbit jelölésére.
