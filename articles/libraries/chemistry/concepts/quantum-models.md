...
Pontosabban, a $N _e $ elektronok, a Momenta $p _i $ (három dimenzióban) és a Mass $m _e $ és a $x _i $ értékű magok, valamint a $Z _K a Hamilton operátor beolvassa a \begin{Equation} \hat{H} = \sum\_{i = 1} ^ {N\_e} \frac{\hat{p}\_i ^ 2} {2m\_e} + \frac{1}{2}\sum\_{i\ne j} \frac{e ^ 2} {| \hat{x}\_i-\hat{x}\_|}-\sum\_{i , k} \frac{Z\_ke ^ 2} {| \hat{x}\_i-{y}\_k |} + \frac{1}{2} \ sum_ {k\ne k '} \frac{Z\_kZ\_{k '} e ^ 2} {| y\_k-y\_k ' |}. \label{EQ: Ham} \end{Equation} a Momenta operátorok $ \hat{p}\_i ^ 2 $ megtekinthetők valós térben, mint Laplacian operátorok, azaz $ \hat{p}\_i ^ 2 =-\partial\_{x\_i} ^ 2-\partial\_{y\_i} ^ 2-\partial\_{z\_i} ^ 2 $.
