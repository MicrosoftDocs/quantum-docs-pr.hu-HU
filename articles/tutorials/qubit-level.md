---
title: 'Qubit programok írása és szimulálása a-ben :::no-loc(Q#):::'
...
<span data-ttu-id="7c59f-105">Bár :::no-loc(Q#)::: elsődlegesen a nagy léptékű kvantum-programok magas szintű programozási nyelve lett létrehozva, a kvantum-programok alacsonyabb szintjének feltárására is használható: közvetlenül az adott qubits kezelése.</span><span class="sxs-lookup"><span data-stu-id="7c59f-105">Although :::no-loc(Q#)::: was primarily created as a high-level programming language for large-scale quantum programs, it can just as easily be used to explore the lower level of quantum programs: directly addressing specific qubits.</span></span>
<span data-ttu-id="7c59f-106">A rugalmasság :::no-loc(Q#)::: lehetővé teszi, hogy a felhasználók bármilyen ilyen szintű absztrakciót megközelítsék a kvantum-rendszereket, és ebben az oktatóanyagban magukra a qubits.</span><span class="sxs-lookup"><span data-stu-id="7c59f-106">The flexibility of :::no-loc(Q#)::: allows users to approach quantum systems from any such level of abstraction, and in this tutorial we dive into the qubits themselves.</span></span>
...
<span data-ttu-id="7c59f-110">Ebben az esetben egy műveletet határozunk meg :::no-loc(Q#)::: a teljes három qubit Quantum Fourier-transzformáció végrehajtásához, amely az alábbi, áramkörként szolgáló módon működik:</span><span class="sxs-lookup"><span data-stu-id="7c59f-110">In our case, we will define a :::no-loc(Q#)::: operation to perform the full three-qubit quantum Fourier transform, which has the following representation as a circuit:</span></span>
...
> [!div class="checklist"]
> * <span data-ttu-id="7c59f-115">Adja meg a kvantum-műveleteket a ben :::no-loc(Q#):::</span><span class="sxs-lookup"><span data-stu-id="7c59f-115">Define quantum operations in :::no-loc(Q#):::</span></span>
> * <span data-ttu-id="7c59f-116">Műveletek meghívása :::no-loc(Q#)::: közvetlenül a parancssorból vagy egy klasszikus gazda program használatával</span><span class="sxs-lookup"><span data-stu-id="7c59f-116">Call :::no-loc(Q#)::: operations directly from the command prompt or using a classical host program</span></span>
...
1. <span data-ttu-id="7c59f-120">Maga a program, amely a Quantum programozási nyelv használatával lett megvalósítva :::no-loc(Q#)::: , majd egy kvantum-számítógépen vagy kvantum-szimulátoron futtatva fut.</span><span class="sxs-lookup"><span data-stu-id="7c59f-120">The program itself, which is implemented using the :::no-loc(Q#)::: quantum programming language, and then invoked to run on a quantum computer or quantum simulator.</span></span> <span data-ttu-id="7c59f-121">Ezek a következőkből állnak</span><span class="sxs-lookup"><span data-stu-id="7c59f-121">These consist of</span></span> 
    - <span data-ttu-id="7c59f-122">:::no-loc(Q#)::: műveletek: a kvantum-regisztráción alapuló alrutinok, és</span><span class="sxs-lookup"><span data-stu-id="7c59f-122">:::no-loc(Q#)::: operations: subroutines acting on quantum registers, and</span></span> 
    - <span data-ttu-id="7c59f-123">:::no-loc(Q#)::: függvények: a kvantum-algoritmuson belül használt klasszikus alrutinok.</span><span class="sxs-lookup"><span data-stu-id="7c59f-123">:::no-loc(Q#)::: functions: classical subroutines used within the quantum algorithm.</span></span>
...
<span data-ttu-id="7c59f-128">Az oktatóanyag első része a művelet definiálását jelenti :::no-loc(Q#)::: `Perform3qubitQFT` , amely a Quantum Fourier-transzformációt három qubits hajtja végre.</span><span class="sxs-lookup"><span data-stu-id="7c59f-128">The first part of this tutorial consists of defining the :::no-loc(Q#)::: operation `Perform3qubitQFT`, which performs the quantum Fourier transform on three qubits.</span></span> 
...
<span data-ttu-id="7c59f-130">Az első lépés a :::no-loc(Q#)::: projekt és a fájl létrehozása.</span><span class="sxs-lookup"><span data-stu-id="7c59f-130">The first step is creating your :::no-loc(Q#)::: project and file.</span></span>
...
### <a name="namespaces-to-access-other-no-locq-operations"></a><span data-ttu-id="7c59f-133">Névterek más műveletekhez való hozzáféréshez :::no-loc(Q#):::</span><span class="sxs-lookup"><span data-stu-id="7c59f-133">Namespaces to access other :::no-loc(Q#)::: operations</span></span>
...
<span data-ttu-id="7c59f-135">A meglévő műveletek használatának elvégzéséhez :::no-loc(Q#)::: nyissa meg a megfelelő `Microsoft.Quantum.<>` névtereket.</span><span class="sxs-lookup"><span data-stu-id="7c59f-135">For our operation to make use of existing :::no-loc(Q#)::: operations, we open the relevant `Microsoft.Quantum.<>` namespaces.</span></span>
...
<span data-ttu-id="7c59f-141">:::no-loc(Q#):::A művelet keretében először három qubits-regisztrációt osztunk ki a `using` nyilatkozattal:</span><span class="sxs-lookup"><span data-stu-id="7c59f-141">Within our :::no-loc(Q#)::: operation, we first allocate a register of three qubits with the `using` statement:</span></span>
...
<span data-ttu-id="7c59f-149">:::no-loc(Q#)::: a névtérben már számos alapvető kvantum-kapu található [`Microsoft.Quantum.Intrinsic`](xref:Microsoft.Quantum.Intrinsic) , és ezek nem kivételek.</span><span class="sxs-lookup"><span data-stu-id="7c59f-149">:::no-loc(Q#)::: already contains many basic quantum gates as operations in the [`Microsoft.Quantum.Intrinsic`](xref:Microsoft.Quantum.Intrinsic) namespace, and these are no exception.</span></span> 

<span data-ttu-id="7c59f-150">Egy :::no-loc(Q#)::: műveleten belül a callables meghívására szolgáló utasítások természetesen szekvenciális sorrendben lesznek futtatva.</span><span class="sxs-lookup"><span data-stu-id="7c59f-150">Within a :::no-loc(Q#)::: operation, the statements invoking callables will, of course, be run in sequential order.</span></span>
...
<span data-ttu-id="7c59f-157">:::no-loc(Q#)::: lehetővé teszi a művelet futtatásának feltételét egy vagy több vezérlő qubits.</span><span class="sxs-lookup"><span data-stu-id="7c59f-157">:::no-loc(Q#)::: makes it extremely easy to condition the run of an operation upon one or multiple control qubits.</span></span>
...
<span data-ttu-id="7c59f-169">Ezért befejeztük a Quantum Fourier-transzformáció qubit-műveletének írását a :::no-loc(Q#)::: műveletbe:</span><span class="sxs-lookup"><span data-stu-id="7c59f-169">Hence we have finished writing the qubit-level operations of the quantum Fourier transform into our :::no-loc(Q#)::: operation:</span></span>
...
<span data-ttu-id="7c59f-177">A teljes :::no-loc(Q#)::: fájlnak így kell kinéznie:</span><span class="sxs-lookup"><span data-stu-id="7c59f-177">Your full :::no-loc(Q#)::: file should now look like this:</span></span>
...
<span data-ttu-id="7c59f-178">A :::no-loc(Q#)::: fájl és a művelet befejezése után a Quantum program készen áll a hívásra és a szimulált állapotra.</span><span class="sxs-lookup"><span data-stu-id="7c59f-178">With the :::no-loc(Q#)::: file and operation complete, our quantum program is ready to be called and simulated.</span></span>
...
<span data-ttu-id="7c59f-180">Miután definiálta a :::no-loc(Q#)::: műveletet egy `.qs` fájlban, most meg kell hívnia ezt a műveletet, és figyelnie kell a visszaadott klasszikus adattípusokat.</span><span class="sxs-lookup"><span data-stu-id="7c59f-180">Having defined our :::no-loc(Q#)::: operation in a `.qs` file, we now need to call that operation and observe any returned classical data.</span></span>
<span data-ttu-id="7c59f-181">Egyelőre nem ad vissza semmit (ne felejtse el, hogy a fent megadott művelet visszaadja `Unit` a műveletet), de ha később módosítjuk a :::no-loc(Q#)::: műveletet a mérési eredmények () egy tömbje visszaadására `Result[]` , ezt a megoldást fogjuk kezelni.</span><span class="sxs-lookup"><span data-stu-id="7c59f-181">For now, there isn't anything returned (recall that our operation defined above returns `Unit`), but when we later modify the :::no-loc(Q#)::: operation to return an array of measurement results (`Result[]`), we will address this.</span></span>

<span data-ttu-id="7c59f-182">Míg a :::no-loc(Q#)::: program a meghívásához használt környezetek között mindenütt elérhető, ennek a módja természetesen változhat.</span><span class="sxs-lookup"><span data-stu-id="7c59f-182">While the :::no-loc(Q#)::: program is ubiquitous across the environments used to call it, the manner of doing so will of course vary.</span></span> <span data-ttu-id="7c59f-183">Ezért egyszerűen kövesse a beállításnak megfelelő lapon található utasításokat: az alkalmazásból való munka vagy a :::no-loc(Q#)::: Python-vagy C#-beli gazda program használata.</span><span class="sxs-lookup"><span data-stu-id="7c59f-183">As such, simply follow the instructions in the tab corresponding to your setup: working from the :::no-loc(Q#)::: application or using a host program in Python or C#.</span></span>
...
<span data-ttu-id="7c59f-185">A :::no-loc(Q#)::: program parancssorból való futtatásához csak kis módosításra van szükség a :::no-loc(Q#)::: fájlban.</span><span class="sxs-lookup"><span data-stu-id="7c59f-185">Running the :::no-loc(Q#)::: program from the command prompt requires only a small change to the :::no-loc(Q#)::: file.</span></span>
...
1. <span data-ttu-id="7c59f-192">Először importáljuk a `qsharp` modult, amely regisztrálja a modul-betöltőt az :::no-loc(Q#)::: együttműködési képességhez.</span><span class="sxs-lookup"><span data-stu-id="7c59f-192">First, we import the `qsharp` module, which registers the module loader for :::no-loc(Q#)::: interoperability.</span></span> 
    <span data-ttu-id="7c59f-193">Ez lehetővé teszi :::no-loc(Q#)::: , hogy a névterek (például a `NamespaceQFT` fájlban definiált :::no-loc(Q#)::: ) Python-modulokként jelenjenek meg, amelyből importálhat :::no-loc(Q#)::: műveleteket.</span><span class="sxs-lookup"><span data-stu-id="7c59f-193">This allows :::no-loc(Q#)::: namespaces (e.g. the `NamespaceQFT` we defined in our :::no-loc(Q#)::: file) to appear as Python modules, from which we can import :::no-loc(Q#)::: operations.</span></span>
2. <span data-ttu-id="7c59f-194">Ezután importálja :::no-loc(Q#)::: azokat a műveleteket, amelyeket a rendszer közvetlenül hív meg---ebben az esetben `Perform3qubitQFT` .</span><span class="sxs-lookup"><span data-stu-id="7c59f-194">Then, import the :::no-loc(Q#)::: operations which we will directly invoke---in this case, `Perform3qubitQFT`.</span></span>
    <span data-ttu-id="7c59f-195">Csak a beléptetési pontot kell importálnia egy :::no-loc(Q#)::: programba (például _nem_ olyan műveletekre, mint a `H` és `R1` , amelyeket más műveletek hívnak meg, :::no-loc(Q#)::: de a klasszikus gazdagép soha nem).</span><span class="sxs-lookup"><span data-stu-id="7c59f-195">We need only import the entry point into a :::no-loc(Q#)::: program (i.e. _not_ operations like `H` and `R1`, which are called by other :::no-loc(Q#)::: operations but never by the classical host).</span></span>
3. <span data-ttu-id="7c59f-196">A :::no-loc(Q#)::: műveletek vagy függvények szimulálása során az űrlap használatával `<:::no-loc(Q#):::callable>.simulate(<args>)` futtassa őket a `QuantumSimulator()` célszámítógépen.</span><span class="sxs-lookup"><span data-stu-id="7c59f-196">In simulating :::no-loc(Q#)::: operations or functions, use the form `<:::no-loc(Q#):::callable>.simulate(<args>)` to run them on the `QuantumSimulator()` target machine.</span></span> 

> [!NOTE]
> <span data-ttu-id="7c59f-197">Ha azt szeretnénk, hogy egy másik gépen, például a-ben hívjuk a műveletet, `ResourceEstimator()` egyszerűen használjuk a következőt: `<:::no-loc(Q#):::callable>.estimate_resources(<args>)` .</span><span class="sxs-lookup"><span data-stu-id="7c59f-197">If we wanted to call the operation on a different machine, for example the `ResourceEstimator()`, we would simply use `<:::no-loc(Q#):::callable>.estimate_resources(<args>)`.</span></span>
> <span data-ttu-id="7c59f-198">Általánosságban elmondható, :::no-loc(Q#)::: hogy a műveletek a futtatott gépektől függetlenek, de bizonyos funkciók, például `DumpMachine` eltérően viselkednek.</span><span class="sxs-lookup"><span data-stu-id="7c59f-198">In general, :::no-loc(Q#)::: operations are agnostic to the machines on which they're run, but some features such as `DumpMachine` may behave differently.</span></span>

4. <span data-ttu-id="7c59f-199">A szimuláció végrehajtásakor a művelet hívása a fájlban meghatározott értékeket ad vissza :::no-loc(Q#)::: .</span><span class="sxs-lookup"><span data-stu-id="7c59f-199">Upon performing the simulation, the operation call will return values as defined in the :::no-loc(Q#)::: file.</span></span>
...
3. <span data-ttu-id="7c59f-211">A kvantumalgoritmus futtatása.</span><span class="sxs-lookup"><span data-stu-id="7c59f-211">Run the quantum algorithm.</span></span> 
    <span data-ttu-id="7c59f-212">Minden :::no-loc(Q#)::: művelet létrehoz egy azonos nevű C#-osztályt.</span><span class="sxs-lookup"><span data-stu-id="7c59f-212">Each :::no-loc(Q#)::: operation generates a C# class with the same name.</span></span> 
...
<span data-ttu-id="7c59f-249">A programokon belüli mérések megvalósításához :::no-loc(Q#)::: a `M` (kezdő) műveletet használjuk `Microsoft.Quantum.Intrinsic` , amely egy `Result` típust ad vissza.</span><span class="sxs-lookup"><span data-stu-id="7c59f-249">To implement measurements within a :::no-loc(Q#)::: program, we use the `M` operation (from `Microsoft.Quantum.Intrinsic`), which returns a `Result` type.</span></span>
...
> [!NOTE]
> <span data-ttu-id="7c59f-259">Az utasítás szintaxisa egyedi :::no-loc(Q#)::: , de megfelel a hasonló változó ismételt hozzárendelésének, `resultArray[i] <- M(qs[i])` más nyelveken, például F # és R nyelven.</span><span class="sxs-lookup"><span data-stu-id="7c59f-259">The syntax of this statement is unique to :::no-loc(Q#):::, but corresponds to the similar variable reassignment `resultArray[i] <- M(qs[i])` seen in other languages such as F# and R.</span></span>
...
<span data-ttu-id="7c59f-271">Ha jobban meg szeretné ismerni a visszaadott tömböt, amelyet a konzolon fog kinyomtatni, a `Message` következő utasítás előtt hozzáadhat egy másikat a :::no-loc(Q#)::: fájlhoz `return` :</span><span class="sxs-lookup"><span data-stu-id="7c59f-271">To have more understanding of the returned array which will be printed in the console, we can add another `Message` in the :::no-loc(Q#)::: file just before the `return` statement:</span></span>
...
## <a name="use-the-no-locq-libraries"></a><span data-ttu-id="7c59f-292">A :::no-loc(Q#)::: kódtárak használata</span><span class="sxs-lookup"><span data-stu-id="7c59f-292">Use the :::no-loc(Q#)::: libraries</span></span>
<span data-ttu-id="7c59f-293">Ahogy azt a bevezetésben is említettük, a nagy része az, :::no-loc(Q#)::: hogy lehetővé teszi, hogy elvonta az egyes qubits foglalkozó aggodalmakat.</span><span class="sxs-lookup"><span data-stu-id="7c59f-293">As we mentioned in the introduction, much of :::no-loc(Q#):::'s power rests in the fact that it allows you to abstract-away the worries of dealing with individual qubits.</span></span>
...
<span data-ttu-id="7c59f-295">A :::no-loc(Q#)::: kódtárak tartalmazzák a [QFT](xref:Microsoft.Quantum.Canon.QFT) műveletet, amely egyszerűen elvégezhető, és tetszőleges számú qubits alkalmazható.</span><span class="sxs-lookup"><span data-stu-id="7c59f-295">The :::no-loc(Q#)::: libraries contain the [QFT](xref:Microsoft.Quantum.Canon.QFT) operation, which you can simply take and apply for any number of qubits.</span></span>
<span data-ttu-id="7c59f-296">A kipróbáláshoz adjon meg egy új műveletet a :::no-loc(Q#)::: fájlban, amelynek a tartalma megegyezik `Perform3QubitQFT` , de az elsőtől `H` a `SWAP` helyébe két egyszerű sor lép:</span><span class="sxs-lookup"><span data-stu-id="7c59f-296">To give it a try, define a new operation in your :::no-loc(Q#)::: file which has the same contents of `Perform3QubitQFT`, but with everything from the first `H` to the `SWAP` replaced by two easy lines:</span></span>
```qsharp
...
<span data-ttu-id="7c59f-299">Ahhoz, hogy hozzáférhessenek ezekhez a műveletekhez, adja hozzá a `open` megfelelő névterekhez tartozó utasításokat a fájl elejéhez :::no-loc(Q#)::: :</span><span class="sxs-lookup"><span data-stu-id="7c59f-299">To have access to these operations, add `open` statements for their respective namespaces at the beginning of the :::no-loc(Q#)::: file:</span></span>
```qsharp
...
<span data-ttu-id="7c59f-301">Ha szeretné megtekinteni a :::no-loc(Q#)::: könyvtári műveletek valódi előnyeit, módosítsa a qubits számát a következőre `3` :</span><span class="sxs-lookup"><span data-stu-id="7c59f-301">To see the real benefit of using the :::no-loc(Q#)::: library operations, change the number of qubits to something other than `3`:</span></span>
```qsharp
