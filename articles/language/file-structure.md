...
## <a name="internal-declarations"></a>Belső deklarációk

A felhasználó által definiált típusok, műveletek és függvények *belsőként*is deklarálva lehetnek.
Ez azt jelenti, hogy csak azok a Q # projektből érhetők el, amelyekben deklarálva vannak.
Ha egy projekt referenciáként van használatban, az összes *nyilvános* (nem belső) deklaráció elérhetővé válik, de egy másik projektből származó belső deklaráció használatakor hiba történik.
A belső deklarációk olyan moduláris kódok írásához hasznosak, amelyeket a projekt más részei is felhasználhatnak, de később továbbra is megváltoztathatók anélkül, hogy más projekteket kellene megszakítani.

Egy belső, felhasználó által definiált típus, művelet vagy függvény egyszerűen deklarálható a deklaráció elején `internal` hozzáadásával.
Például:
...
> [!WARNING]
> A belső felhasználó által definiált típusok csak aláírásokban vagy alapul szolgáló típusokban használhatók, ha a megfelelő hívható vagy felhasználó által definiált típus is belső.
> Ha például van egy felhasználó által definiált `InternalOptions`, amely a `internal` kulcsszóval van deklarálva, akkor a következő deklarációk hibát eredményeznek:
>
> ```qsharp
