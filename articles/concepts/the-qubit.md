...
Most, hogy már tudjuk, hogyan jelentheti a qubit, megtudhatjuk, hogy mit jelentenek ezek az államok a [*mérés*](https://en.wikipedia.org/wiki/Measurement_in_quantum_mechanics)koncepciójának megvitatása során. Egy mérték megfelel a "looking" (keresés) qubit, amely azonnal Összecsukja a kvantum-állapotot a két klasszikus állapot közül a $ \begin{bmatrix} 1 \\ \\ 0 \end{bmatrix} $ vagy $ \begin{bmatrix} 0 \\ \\ 1 \end{bmatrix} $ értékkel. Ha a Quantum State Vector $ \begin{bmatrix} \alpha \\ \\ \beta \end{bmatrix} $ által megadott qubit mérik, a $0 $ értéket kapjuk meg valószínűséggel $ | \alpha | ^ 2 $, valamint a $1 $ értékkel, valószínűség: $ | \beta | ^ 2 $. A $0 $ értéknél a qubit új állapota $ \begin{bmatrix} 1 \\ \\ 0 \end{bmatrix} $; a $1 $ állapotának eredménye $ \begin{bmatrix} 0 \\ \\ 1 \end{bmatrix} $. Vegye figyelembe, hogy ezek a valószínűségek összege $1 $, a normalizálás feltétele miatt $ | \alpha | ^ 2 + | \beta | ^ 2 = $1.

A mérés tulajdonságai azt is jelenti, hogy a kvantum-állapot vektorának általános aláírása lényegtelen. A vektor megtagadásával egyenértékű a $ \alpha \rightarrow-\alpha $ és a $ \beta \rightarrow-\beta $ értékkel. Mivel a $0 $ és a $1 $ mérés valószínűsége a feltételek méretétől függ, az ilyen jelek beillesztése nem változtatja meg a valószínűségeket. Az ilyen fázisokat általában [ `` *globális fázisoknak*](https://en.wikipedia.org/wiki/Phase_factor) nevezik, és általánosabban $e ^ {i \phi} $, nem csupán $ \pm $1.

A mérés utolsó fontos tulajdonsága, hogy nem szükségszerűen károsítja az összes kvantum-állapotú vektort. Ha a qubit a \begin{bmatrix} 1 \\ \\ 0 \end{bmatrix} $, amely a klasszikus állapot $0 $-ként felel meg, akkor ez az állapot mindig a $0 $ értéket adja eredményül, és változatlanul hagyja a kvantum-állapotot. Ebben az értelemben, ha csak klasszikus bitek vannak (azaz a $ \begin{bmatrix}1 \\ \\ 0 \end{bmatrix} $ vagy $ \begin{bmatrix}0 \\ \\ 1 \end{bmatrix} $ qubits), akkor a mérés nem károsítja a rendszerét. Ez azt jelenti, hogy a klasszikus adatmennyiséget replikálhatja, és a kvantum-számítógépeken ugyanúgy kezelheti azt, mint egy klasszikus számítógépen. Ugyanakkor az információk mindkét állapotban való tárolása egyszerre történik, ami túlmutat a kvantum-számítástechnika a klasszikusan lehetségesnél, és tovább megfosztja a kvantum-számítógépeket a Quantum-adatok válogatás nélküli másolásának lehetőségével, lásd még [a nem klónozási tétel](https://en.wikipedia.org/wiki/No-cloning_theorem).
...
Bár ez egy triviális példa (a <xref:microsoft.quantum.intrinsic.h> művelet önadjoint), megtekintheti, hogy ez milyen felbecsülhetetlen értékű qubit-műveletekhez válik.
...
Más szóval, a Gates egy univerzális Gate-készlet, ha bármely egységes átalakítás a készletből származó Gates-termékként is írható. Szükség van arra, hogy az előírt hibákhoz kötve legyenek a Gates $G _ {1} , {2} a G_, a \ldots, a G_N $ a Gate készletből, hogy

$ $ G_N G_ {N-1} \cdots G_2 G_1 \approx U. $ $

Vegye figyelembe, hogy mivel a mátrixok szorzási konvenciója a jobbról az első kapu műveletének $G _N $ értéket adja meg, valójában az utolsó, amelyet a kvantum-állapot vektorára alkalmaztak. További formálisan azt mondjuk, hogy egy ilyen Gate-készlet univerzális, ha minden hibatűrés $ \epsilon>$0 létezik $G _1, \ldots, G_N $, hogy a $G _N \ldots G_1 $ és a $U $ közötti távolság legfeljebb $ \epsilon $. Ideális esetben $N $ $ \epsilon $ eléréséhez szükséges értéknek kell megfelelnie a Poly-logaritmikusan és a $1/\ epszilon $ közötti méretnek.
