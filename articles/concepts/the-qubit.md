...
Most, hogy már tudjuk, hogyan jelentheti a qubit, megtudhatjuk, hogy mit jelentenek ezek az államok a [*mérés*](https://en.wikipedia.org/wiki/Measurement_in_quantum_mechanics)koncepciójának megvitatása során. Egy mérték megfelel a "looking" qubit, amely azonnal Összecsukja a kvantum-állapotot a két klasszikus állapot közül a $ \begin{bmatrix} 1 \\\\ 0 \end{bmatrix} $ vagy a $ \begin{bmatrix} 0 \\\\ 1 \end{ bmatrix} $. Ha a Quantum State Vector $ \begin{bmatrix} \alpha \\\\ \beta \end{bmatrix} $ értékkel megadott qubit mérik, a $0 $ értéket kapjuk meg, a valószínűség: $ | \alpha | ^ 2 $, valamint a $1 $ értéket a valószínűség $ | \beta | ^ 2 $. A $0 $ értéknél a qubit új állapota $ \begin{bmatrix} 1 \\\\ 0 \end{bmatrix} $; $1 $ után az állapota $ \begin{bmatrix} 0 \\\\ 1 \end{bmatrix} $. Vegye figyelembe, hogy ezek a valószínűségek összege $1 $, a normalizálás feltétele miatt $ | \alpha | ^ 2 + | \beta | ^ 2 = $1.
...
Más szóval, a Gates egy univerzális Gate-készlet, ha bármely egységes átalakítás a készletből származó Gates-termékként is írható. Megköveteljük, hogy az előírt hibákhoz kötve legyenek a Gates $G _{1}, a G_{2}, a \ldots, a G_N $ a Gate készletből, hogy

$ $ G_N G_ {N-1} \cdots G_2 G_1 \approx U. $ $

Vegye figyelembe, hogy mivel a mátrixok szorzási konvenciója az, hogy az első kapu műveletét az ebben a sorozatban, $G _N $ értéket adja meg, valójában az utolsó lett alkalmazva a kvantum-állapot vektorára. További formálisan azt mondjuk, hogy egy ilyen Gate-készlet univerzális, ha minden hibatűrés $ \epsilon > 0 $ létezik $G _1, \ldots, G_N $, hogy a $G _N \ ldots G_1 $ és $U $ közötti távolság legfeljebb $ \epsilon $. Ideális esetben $N $ $ \epsilon $ eléréséhez szükséges értéknek kell megfelelnie a Poly-logaritmikusan és a $1/\ epszilon $ közötti méretnek.
...
Példa arra, hogy az egységes átalakítások hogyan használhatók ezekből a primitívekről, a fenti Bloch-szférákban látható három átalakítás a \begin{bmatrix} 1. \\\\ 0 \end{bmatrix} \mapsto HZH \begin{bmatrix} 1 \\\\ 0 \end{bmatrix} = \begin{bmatrix} 0 \\\\ 1 \end{bmatrix} $.
