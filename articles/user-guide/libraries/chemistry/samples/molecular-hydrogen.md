...
<span data-ttu-id="b87b8-122">Most meghívhatja a Q# kódot a gazda programból.</span><span class="sxs-lookup"><span data-stu-id="b87b8-122">You can now invoke the Q# code from the host program.</span></span> <span data-ttu-id="b87b8-123">A következő C#-kód egy teljes állapotú szimulátort hoz létre `GetEnergyByTrotterization` , amely a terepi állapot megszerzéséhez szükséges.</span><span class="sxs-lookup"><span data-stu-id="b87b8-123">The following C# code creates a full-state simulator and runs `GetEnergyByTrotterization` to obtain the ground state energy.</span></span>
