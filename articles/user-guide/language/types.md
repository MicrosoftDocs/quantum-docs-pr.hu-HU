...
---
# <a name="types-in-no-locq"></a><span data-ttu-id="6d241-103">Típusok Q#</span><span class="sxs-lookup"><span data-stu-id="6d241-103">Types in Q#</span></span>

<span data-ttu-id="6d241-104">Ez a cikk a Q# típus modelljét és a típusok megadásának és használatának szintaxisát ismerteti.</span><span class="sxs-lookup"><span data-stu-id="6d241-104">This article describes the Q# type model and the syntax for specifying and working with types.</span></span> <span data-ttu-id="6d241-105">Az ilyen típusú kifejezések létrehozásával és működésével kapcsolatos részletekért lásd: [kifejezések megadása](xref:microsoft.quantum.guide.expressions).</span><span class="sxs-lookup"><span data-stu-id="6d241-105">For details on how to create and operate on expressions of these types, see [Type Expressions](xref:microsoft.quantum.guide.expressions).</span></span>

<span data-ttu-id="6d241-106">Fontos megjegyezni, hogy ez Q# egy *erősen gépelt* nyelv, amely az ilyen típusú alkalmazások körültekintő használatát segíti a fordítót, hogy erős garanciát nyújtson a Q# programoknak a fordítás ideje alatt.</span><span class="sxs-lookup"><span data-stu-id="6d241-106">We note that Q# is a *strongly-typed* language, such that careful use of these types can help the compiler to provide strong guarantees about Q# programs at compile time.</span></span>
<span data-ttu-id="6d241-107">A legerősebb garanciák biztosításához a típusok közötti konverziónak explicit módon kell megadnia a Q# függvényeknek a konverziót kifejező hívásait.</span><span class="sxs-lookup"><span data-stu-id="6d241-107">To provide the strongest guarantees possible, conversions between types in Q# must be explicit using calls to functions which express that conversion.</span></span> 
<span data-ttu-id="6d241-108">Q# számos ilyen funkciót biztosít a névtér részeként <xref:Microsoft.Quantum.Convert> .</span><span class="sxs-lookup"><span data-stu-id="6d241-108">Q# provides a variety of such functions as a part of the <xref:Microsoft.Quantum.Convert> namespace.</span></span>
...
<span data-ttu-id="6d241-110">Q# a a közvetlen használatú primitív típusokat, valamint számos különböző módszert biztosít a más típusú új típusok létrehozásához.</span><span class="sxs-lookup"><span data-stu-id="6d241-110">Q# provides both primitive types, which are used directly, and a variety of ways to produce new types from other types.</span></span>
...
<span data-ttu-id="6d241-113">A Q# nyelv a következő *primitív típusokat* biztosítja, amelyek mindegyike közvetlenül a Q# programokban használható.</span><span class="sxs-lookup"><span data-stu-id="6d241-113">The Q# language provides the following *primitive types* , all of which you can use directly in Q# programs.</span></span> <span data-ttu-id="6d241-114">Ezeket a primitív típusokat új típusok létrehozására is használhatja.</span><span class="sxs-lookup"><span data-stu-id="6d241-114">You can also use these primitive types to construct new types.</span></span>
...
- <span data-ttu-id="6d241-125">A `Unit` típusnak csak egy értéke lehet `()` .</span><span class="sxs-lookup"><span data-stu-id="6d241-125">The `Unit` type can have only one value, `()`.</span></span> 
  <span data-ttu-id="6d241-126">Ezzel a típussal jelezheti, hogy egy Q# függvény vagy művelet nem ad vissza adatokat.</span><span class="sxs-lookup"><span data-stu-id="6d241-126">Use this type to indicate that a Q# function or operation returns no information.</span></span> 
...
<span data-ttu-id="6d241-137">A konstansok,,,,,,, `true` `false` és az `PauliI` `PauliX` `PauliY` `PauliZ` `One` `Zero` összes foglalt szimbólum szerepel a alkalmazásban Q# .</span><span class="sxs-lookup"><span data-stu-id="6d241-137">The constants `true`, `false`, `PauliI`, `PauliX`, `PauliY`, `PauliZ`, `One`, and `Zero` are all reserved symbols in Q#.</span></span>
...
* <span data-ttu-id="6d241-139">Bármely érvényes Q# típus esetében létezik egy típus, amely az adott típusú értékek tömbjét jelöli.</span><span class="sxs-lookup"><span data-stu-id="6d241-139">For any valid Q# type, there is a type that represents an array of values of that type.</span></span>
...
> [!NOTE] 
> <span data-ttu-id="6d241-143">Ez a példa `(Bool, Pauli)[][]` a tömbök potenciálisan szaggatott tömbjét jelöli, nem pedig egy téglalap alakú kétdimenziós tömböt.</span><span class="sxs-lookup"><span data-stu-id="6d241-143">This example, `(Bool, Pauli)[][]`, represents a potentially jagged array of arrays and not a rectangular two-dimensional array.</span></span> <span data-ttu-id="6d241-144">Q# a nem támogatja a négyszögletes többdimenziós tömböket.</span><span class="sxs-lookup"><span data-stu-id="6d241-144">Q# does not support rectangular multi-dimensional arrays.</span></span>

* <span data-ttu-id="6d241-145">A tömb értéke a Q# forráskódban a tömb elemei körül szögletes zárójelek használatával is írható `[PauliI, PauliX, PauliY, PauliZ]` .</span><span class="sxs-lookup"><span data-stu-id="6d241-145">An array value can be written in Q# source code by using square brackets around the elements of an array, as in `[PauliI, PauliX, PauliY, PauliZ]`.</span></span>
...
<span data-ttu-id="6d241-156">A rekordok egy hatékony koncepció, amellyel Q# az értékek összegyűjthetők egyetlen értékkel, így könnyebben átadhatók.</span><span class="sxs-lookup"><span data-stu-id="6d241-156">Tuples are a powerful concept used throughout Q# to collect values together into a single value, making it easier to pass them around.</span></span>
...
* <span data-ttu-id="6d241-164">A 0,3-as és az Q# `Unit` üres rekord *típusának* neve az `()` üres rekord *értékéhez* használatos.</span><span class="sxs-lookup"><span data-stu-id="6d241-164">As of Q# 0.3, `Unit` is the name of the *type* of the empty tuple; `()` is used for the *value* of the empty tuple.</span></span>
...
<span data-ttu-id="6d241-166">Q# a nem biztosít olyan mechanizmust, amely a létrehozott rekord tartalmát egyszer módosítja.</span><span class="sxs-lookup"><span data-stu-id="6d241-166">Q# does not provide a mechanism to change the contents of a tuple once created.</span></span>
...
<span data-ttu-id="6d241-169">Q#Az Egypéldányos rekordokat azonban a befoglalt típus értékével egyenértékűként kezeli.</span><span class="sxs-lookup"><span data-stu-id="6d241-169">However, Q# treats a singleton tuple as equivalent to a value of the enclosed type.</span></span>
...
* <span data-ttu-id="6d241-180">Q#Előfordulhat, hogy minden forrásfájl tetszőleges számú felhasználó által definiált típust deklarál.</span><span class="sxs-lookup"><span data-stu-id="6d241-180">Each Q# source file may declare any number of user-defined types.</span></span>
...
<span data-ttu-id="6d241-185">Egy Q# fájl meghatározhatja a jogi típusok egyetlen értékét tartalmazó új elnevezett típust.</span><span class="sxs-lookup"><span data-stu-id="6d241-185">A Q# file may define a new named type containing a single value of any legal type.</span></span>
...
<span data-ttu-id="6d241-197">Bár mindkettő `Complex` és `ComplexPolar` mindkettő rendelkezik egy alapul szolgáló típussal `(Double, Double)` , a két típus teljesen inkompatibilis a-ben Q# , ami minimalizálja annak kockázatát, hogy véletlenül egy összetett matematikai függvényt hív meg a Polar koordinátákkal, és fordítva.</span><span class="sxs-lookup"><span data-stu-id="6d241-197">Even though both `Complex` and `ComplexPolar` both have an underlying type `(Double, Double)`, the two types are wholly incompatible in Q#, minimizing the risk of accidentally calling a complex math function with polar coordinates and vice versa.</span></span>
...
<span data-ttu-id="6d241-205">További információ a kicsomagolási operátorról: [kifejezések beírása Q# a alkalmazásban ](xref:microsoft.quantum.guide.expressions).</span><span class="sxs-lookup"><span data-stu-id="6d241-205">For more details on the unwrap operator, see [Type Expressions in Q#](xref:microsoft.quantum.guide.expressions).</span></span>
...
* <span data-ttu-id="6d241-222">Minden Q# callables egyetlen értéket adjon meg bemenetként, és egyetlen értéket ad vissza kimenetként.</span><span class="sxs-lookup"><span data-stu-id="6d241-222">All Q# callables take a single value as input and return a single value as output.</span></span>
...
<span data-ttu-id="6d241-244">A típus-paraméteres callables definiálásával kapcsolatos további információkért lásd: [műveletek és függvények a alkalmazásban Q# ](xref:microsoft.quantum.guide.operationsfunctions#generic-type-parameterized-callables).</span><span class="sxs-lookup"><span data-stu-id="6d241-244">For more information and details on how to define type-parameterized callables, see [Operations and Functions in Q#](xref:microsoft.quantum.guide.operationsfunctions#generic-type-parameterized-callables).</span></span>
...
<span data-ttu-id="6d241-249">Q# a nem biztosít olyan mechanizmust, amely lehetővé teszi, hogy a felhasználók egy típusparaméter esetében helyettesíthetik a lehetséges típusokat.</span><span class="sxs-lookup"><span data-stu-id="6d241-249">Q# does not provide a mechanism for constraining the possible types that a user might substitute for a type parameter.</span></span>
