...
Ez lehetővé teszi az IntelliSense-funkciók új készletének használatát, valamint az élő visszajelzéseket, amelyekkel squiggly a hibák és figyelmeztetések beírásával.
...
Ez a frissítés nagy mértékben javítja a diagnosztikai üzeneteket, és az egyszerű navigálás és a pontos tartomány a diagnosztika és a további részletek a megjelenített hover-információkban. 
...
Ahhoz, hogy a megoldás működjön a Visual Studióban, a megoldásban szereplő összes projektnek ugyanabban a mappában kell lennie, mint a megoldásnak, vagy az egyik almappájában.
...
A feltételes kifejezés a `condition ? caseTrue | caseFalse`űrlap kifejezése.
Ahogy a fenti példában is látható, a tömbök iterációja mostantól támogatott.

A `using`on és `borrowing`on belüli foglalások újonnan lehetségesek egyetlen qubits, qubit tömbökben és beágyazott rekordok:
```qsharp
    using (q = Qubit()) {
...
Két új specializációs létrehozó direktíva `invert` és `distribute` a `auto`eken kívül a rendszer bevezeti a rendszerállapot-specializációk generálásának szabályozását.
A műveleteken belüli alapértelmezett `body` specializáció explicit deklarációja már nem kötelező.
...
A felhasználó által definiált típusok kezelésének jelentős változása a 0,3-es frissítéssel jár. Míg a korábbi kiadásokban a felhasználó által definiált típusokat a rendszer az alapul szolgáló típus altípusai szerint kezeli [](#BreakingChanges)  
...
`Adjoint`, `Controlled`és egy új "kicsomagolási" Postfix operátor `!` kezeli őket kifejezés-Módosítóként. A `Adjoint` és az `Controlled`, mint a kifejezés-módosítók kezelése sok esetben szükségtelenné teszi a zárójelek számát. Az alábbi kifejezések például érvényesek `Op`, egy egységes művelet, `opArr` az egységes műveletek tömbje, és `arg` a megfelelő argumentum: 
```qsharp
    Adjoint Op (arg);
    Controlled opArr[i] (arg);
```
Az áttelepítési parancsfájl a felesleges zárójelek többségét elvonja, és teljes mértékben kihasználja az új funkciókat. Javasoljuk azonban, hogy a parancsfájl ne értelmezze a kódot, és nem fogja figyelembe venni a ritka esetekben a `Controlled`-leírók különböző értelmezését (csak a részleges alkalmazásokkal kapcsolatos releváns).
...
```
Ahogy a példában látható, `!` kötések szigorúbbak, mint például a `Controlled` és a `Adjoint`.

#<a name="BreakingChanges"></a>Változtatások megszakítása #
...
Az alább felsorolt változásokon kívül a minta `__*__`, ahol * a nem szóközöket tartalmazó sorok a belső használatra vannak fenntartva, és nem használhatók szimbólum neveként.  
...
Az alábbi példa azt szemlélteti, hogyan lehet a `ToOperation` függvény használatával módosítani egy függvény `Square` egy művelet `op`: 
```qsharp
...
#<a name="Migration"></a>Meglévő projektek migrálása Q # 0,3-re #
...
1. A parancssorból futtassa a `dotnet clean`t az összes meglévő bináris és köztes fájl eltávolításához.
2. Egy szövegszerkesztőben szerkessze a. csproj fájlt, és módosítsa az összes "Microsoft. Quantum" `PackageReference` verzióját a 0.3.1811.2802-Preview verzióra, például:
```xml
...
```
4. A parancssorban használja a parancssori fordítóba integrált formázó eszközt a következő parancs futtatásával: `dotnet msbuild /t:qsharpformat`  
    - A fájlok helyben lesznek áttelepítve. A rendszer átmásolja az összes eredeti fájl biztonsági másolatát `obj\qsharp\.backup`
...
5. A futtatása után továbbra is manuálisan kell megadnia a szemantikai változásokat olyan esetekben, amikor a kód szemantikai értelmezése megváltozott. Ezeket a hibákat az IntelliSense a Visual Studióban vagy a Visual Studio Code-ban fogja jelenteni.
    ...
    - Miután a projekt sikeresen betöltődött (a kimenet ablakban látható), nyissa meg az egyes fájlokat, és manuálisan végezze el az összes fennmaradó probléma megoldását.
...
> * Ha egy projekttel szeretne dolgozni a Visual Studio Code-ban, nyissa meg a projektet tartalmazó legfelső szintű mappát és az összes hivatkozott projektet.   
> * Ahhoz, hogy a megoldás működjön a Visual Studióban, a megoldásban szereplő összes projektnek ugyanabban a mappában kell lennie, mint a megoldásnak, vagy az egyik almappájában.  
...
##<a name="MigrationScript"></a>Áttelepítési parancsfájl ##
...
- A projekt a Quantum Development Kit korábbi verziójával készült, így biztosítva, hogy a Migrálás valószínűleg sikeres legyen. Ez a lépés kihagyható a `-Force` paraméterrel a parancsfájlhoz.
...
> Ha engedélyezni szeretné a parancsfájlok, például a `qdk-migrate.ps1` futtatását a Windows rendszeren, lehetséges, hogy módosítania kell a végrehajtási házirendet.
